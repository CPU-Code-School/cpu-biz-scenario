...
create table dbo.Flight(
    ...
    constraint ck_Flight_passport_expiry_date_is_9_years_and_six_months_after_passport_issue_date_for_those_that_are_sixteen_and_5_years_for_under_16_and_that_it_is_valid_to_fly
        check((datediff(year, DOB, PassportIssueDate) >= 16 and datediff(month, PassportIssueDate, PassportExpiryDate) <= 114 and (((DepartureCountry = ArrivalCountry or PassportNationality = ArrivalCountry) and TimeDeparting <= PassportExpiryDate) 
        ...
        or ((datediff(year, DOB, PassportIssueDate) < 16 and datediff(year, PassportIssueDate, PassportExpiryDate) <= 5)) and (((DepartureCountry = ArrivalCountry or PassportNationality = ArrivalCountry) and TimeDeparting <= PassportExpiryDate) 
        or ((DepartureCountry <> ArrivalCountry or PassportNationality <> ArrivalCountry) and datediff(month, TimeDeparting, PassportExpiryDate) > 5))),
