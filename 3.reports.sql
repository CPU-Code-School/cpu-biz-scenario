...
select top 1 TotalCostumeSold = sum(s.Quantity), s.CostumesBought
from sales s
group by s.CostumesBought
order by TotalCostumeSold desc

-- 2. I need to know which size is the most polpular.
select top 1 TotalSizeSold= sum(s.Quantity), s.CostumeSize
from sales s
...
select Customers = concat(s.CustomerFirstName, ' ', CustomerLastName, ': ', s.Quantity, ' - ', s.CostumesBought,' (', s.PriceSoldPerCostume, ')')
from Sales s
