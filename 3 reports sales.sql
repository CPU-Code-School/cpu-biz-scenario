...
from sales s
where s.City = 'Spring Valley'
select OutOfTownCustomers = count (*)
from sales s
where s.City <> 'Spring Valley'
--found this beautiful syntax from another students pull request
select
count(case when City = 'Spring Valley' then 1 end) as LoalCustomers,
count(case when City <> 'Spring Valley' then 1 end) as OutOfTownCustomers
from sales

--or
select
LoalCustomers = count(case when City = 'Spring Valley' then 1 end) ,
OutOfTownCustomers = count(case when City <> 'Spring Valley' then 1 end) 
from sales
