...
from Passenger p 
group by p.FlightNumber, P.DepartureAirport, p.TimeDeparting, p.ArrivalAirport
...
select DepartureDate = convert(varchar, p.TimeDeparting, 103), NumPassengers = Count(*), FlightsDeparting = count(distinct p.FlightNumber)
from Passenger p 
...
select p.ArrivalAirport, NumPassengers = Count(*), NumberOfFlights = count(distinct p.FlightNumber)
from Passenger p 
