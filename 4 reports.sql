...
*/
select SoldierInfo = concat('(2017-2019) ', s.SoldierFirstName, ' ', s.SoldierLastName, ' - ', s.SSN, ' (', s.DateOfEnlistment, '), ', s.ServiceUnit)
from Soldiers s
where year(s.DateOfEnlistment) between 2017 and 2019
union select SoldierInfo = concat('(2020-2022) ', s.SoldierFirstName, ' ', s.SoldierLastName, ' - ', s.SSN, ' (', s.DateOfEnlistment, '), ', s.ServiceUnit)
from Soldiers s
...
select s.ServiceUnit, AvgIQ = avg(s.IQlevel)
from Soldiers s
