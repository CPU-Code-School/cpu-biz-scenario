...
PhoneNumber char(12) not null constraint ck_Bike_phonenumber_must_have_certain_format check(PhoneNumber in '[0-9][0-9][0-9][-][0-9][0-9][0-9][-][0-9][0-9][0-9][0-9]'),
...
New bit not null,
ConditionWhenReceived int null constraint ck_Bike_conditionwhenreceived_must_be_1_or_2_or_3_or_4_or_null check(ConditionWhenReceived in (1,2,3,4)),
...
constraint ck_Bike_conditionwhenreceived_and_New_must_align check((New = 0 and ConditionWhenReceived in (1,2,3,4)) or (New = 1 and (ConditionWhenReceived is null)))
)
